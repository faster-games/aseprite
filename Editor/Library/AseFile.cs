...
using FasterGames.Aseprite.Editor.Library.Utils;
using UnityEditor;
using UnityEngine;
    ...
    public class AseFile
    {
        ...
        public Texture2D GetFrame(int index, FrameOptions opts)
        {
            ...
            for (int i = 0; i < cels.Count; i++)
            {
                ...
                if (!opts.Includes(layer.LayerName)) // ignore filtered
                    continue;
        ...
        {
            Texture2D[] frames = this.GetFrames(opts);
